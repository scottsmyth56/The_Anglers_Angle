...
class IndexViewTests(SetUpClass):
    def setUp(self):
        ...
        Post.objects.create(
            user_id=self.user, title="Test Title 1", content="Test Post 1"
        )
        Post.objects.create(
            user_id=self.user, title="Test Title 2", content="Test Post 2"
        )
...
class CommentTests(SetUpClass):
    ...
    def test_delete_comment_successfully(self):
        self.client.login(username="testuser", password="12345")
        self.comment = Comment.objects.create(
            user_id=self.user, post_id=self.post, comment="Test Comment"
        )
        self.delete_comment_url = reverse(
            "deleteComment", kwargs={"pk": self.comment.pk}
        )
        initial_comment_count = Comment.objects.count()
